...
export default function AgentDashboard({ user }) { 
  ...
  const handleOutcome = async (leadId, outcome) => {
    ...
    try {
      ...
      if (outcome === 'Meeting Booked')
        updates.stage_name = 'Meeting Scheduled';
  ...
  const displayedLeads =
    activeTab === 'tasks'
      ? leads.filter(
          (l) => l.stage_name === 'New Lead' || l.temperature === 'Hot'
        )
      : leads;
      ...
      <aside className="w-[350px] bg-white border-r border-slate-200 flex flex-col shrink-0 z-10">
        <div className="p-6 bg-[#0B0F19] text-white flex justify-between items-center shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-black uppercase tracking-[0.2em] text-sm leading-none mt-1">
              Workspace
            </span>
          </div>
        </div>
            ...
            displayedLeads.map((l) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-sm tracking-tight text-slate-900 leading-tight pr-2">
                    {l.school_name}
                  </h4>
                  {l.temperature === 'Hot' && (
                    <Flame
      ...
      <div className="flex-1 bg-slate-50 flex flex-col relative overflow-hidden min-w-0">
        {!selectedLead ? (
          ...
          <div className="flex-1 flex flex-col animate-in fade-in duration-300">
            <header className="p-8 border-b border-slate-100 flex justify-between items-center bg-white z-10 shadow-sm flex-wrap gap-4">
              <div>
                <h2 className="text-3xl font-black text-slate-900 tracking-tight">
                  {selectedLead.school_name}
                </h2>
                <div className="flex flex-wrap items-center mt-3 gap-4 text-sm font-bold text-slate-500 uppercase tracking-widest">
              ...
              <div className="text-right flex flex-col items-end gap-2">
                <span className="px-4 py-1.5 bg-[#0B0F19] text-white text-[10px] font-black rounded-lg uppercase tracking-widest shadow-sm">
                  {selectedLead.stage_name || 'New'}
                </span>
                <span
                  className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded border ${
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                <div className="flex flex-col gap-6">
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm w-full">
                    <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 border-b border-slate-50 pb-3">
                      Target Profile
                    </h5>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="font-bold text-slate-800 text-xs">
                          {selectedLead.pc3 || 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>

                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center items-center text-center w-full">
                    <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                      Edivy Score
                    </h5>
                    <div className="text-5xl font-black text-indigo-600 tabular-nums leading-none tracking-tighter">
                      {selectedLead.score || 0}
                    </div>
                  </div>
                </div>

                <div className="bg-white p-8 rounded-3xl shadow-md border border-slate-200 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-6 opacity-5">
                    <MessageSquare className="w-32 h-32" />
                  </div>
                  <h3 className="text-[10px] font-black text-indigo-800 uppercase tracking-widest mb-5 flex items-center">
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl text-sm font-medium italic text-slate-600 border border-slate-200 shadow-inner leading-relaxed">
                    "Hi {selectedLead.contact_name}, noticed{' '}
                    {selectedLead.school_name} is managing parent comms via{' '}
                    {selectedLead.pc2 || 'manual methods'}. We help schools
                    automate this. Open to a 10-min chat?"
                  </div>
                  <div className="mt-6 flex gap-4">
                    <button
                      onClick={() =>
                        window.open(
                          `https://wa.me/${selectedLead.phone}?text=Hi%20${selectedLead.contact_name}...`,
                          '_blank'
                        )
                      }
                      className="w-full sm:w-auto bg-emerald-500 text-white font-black py-4 px-6 rounded-xl shadow-md hover:bg-emerald-600 transition-all uppercase text-[10px] tracking-widest flex items-center justify-center"
                    >
                      <Send className="w-4 h-4 mr-2" /> Open WhatsApp
                    </button>
                  </div>
                  <div className="mt-8 pt-6 border-t border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 text-center sm:text-left">
                      Update Outcome
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        <button
                          ...
                          className="px-2 py-3 bg-white border border-slate-200 rounded-lg text-[9px] font-black text-slate-600 hover:bg-indigo-600 hover:text-white transition-all uppercase tracking-widest shadow-sm"
                        >
                          {o}
                        </button>
                      ))}
